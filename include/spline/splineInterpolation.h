...
public:
    ...
    Y firstDerivate(const X&x) const {
        if (mElements.size() == 0) return Y();
        ...
        return it->firstDeriv(x);
    }

    Y secondDerivate(const X&x) const {
        if (mElements.size() == 0) return Y();
...
protected:
    ...
    public:
        ...
        Y firstDeriv(const X& xx) const {
            Y c_coef, d_coef;
            c_coef[0] = c_coef[1] = 2.0;
            d_coef[0] = d_coef[1] = 3.0;

            X xix(xx - x);
            return b + c_coef * c * xix + d_coef * d * (xix * xix);
        }

        Y secondDeriv(const X& xx) const {
            Y c_coef, d_coef;
            c_coef[0] = c_coef[1] = 2.0;
            d_coef[0] = d_coef[1] = 6.0;

            X xix(xx - x);
            return c_coef * c + d_coef * d * xix;
        }
